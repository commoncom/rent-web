<template>
  	<div class="home_container">
        ...
        <div id="bg" class="bg">
          <div class="login">
             <div class="login">
              <div class="logo">欢迎来到转账授权</div>
              <el-form>
                 <el-form-item label="授权地址">
                    <el-input type="text" id="approveAddr" v-model="tokenInfo.approveAddr" @blur="inputBlur('approveAddr',tokenInfo.approveAddr)"></el-input>
                    <p>{{tokenInfo.approveAddrErr}}</p>
                </el-form-item>
                ...
                <el-form-item>
                  <el-button type="primary" @click="submitForm('tokenInfo')" v-bind:disabled="tokenInfo.beDisabled">授权</el-button>
...
export default {
   ...
   methods : {
      ...
      submitForm:function(formInfo){
        ...
        let url = UrlConfig.serverUrl+"/approvetransfer/"+info.approveAddr+"/"+info.amount+"/"+info.addr+"/"+info.prikey;
        console.log(url);
      ...
      inputBlur:function(errorItem,inputContent){
          ...
          if (errorItem === 'approveAddr') {
              if (inputContent === '') {
                  this.tokenInfo.approveAddrErr = '授权地址不能为空！';
                  flag = false;
              } else if (!(addrReg.test(inputContent))) {
                 this.tokenInfo.approveAddrErr = '授权地址不符合规则！';
                 flag = false;
              } else{
                  this.tokenInfo.approveAddrErr = '';
              }
